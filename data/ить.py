...
async def title_confirmed(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "6 11/4 1\n"
        "7 sin(pi/2) 1\n\n"
        "⛔ ИСПОЛЬЗУЙ ТОЛЬКО ДОСТУПНЫЕ СИМВОЛЫ (например: ^ вместо ², pi вместо π, √ вместо sqrt, log_b(x) для логарифмов)"
    )
    ...
    for line in lines:
        try:
            ...
            await message.answer(
                ...
                "4 sqrt(64) 1\n"
                "5 E=mc^2 1.5\n\n"
                "⛔ ИСПОЛЬЗУЙ ТОЛЬКО ДОСТУПНЫЕ СИМВОЛЫ (например: sqrt(9), ^2 вместо ², pi вместо π)"
            )
            return
...
async def redo_questions(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("✏️ Введите вопросы заново в корректном формате.")
    await state.set_state(CreateTestState.waiting_for_questions)
